...
export default function QRGenerator({ data, isVisible }: QRGeneratorProps) {
  ...
  return (
    <motion.div
      ...
      className="flex flex-col items-center gap-8 p-10 glass rounded-[48px] border-white/10 shadow-2xl relative overflow-hidden"
    >
      ...
      <div className="relative group">
        ...
        <div className="bg-white p-6 rounded-3xl shadow-[0_0_50px_rgba(57,255,20,0.1)] relative overflow-hidden">
          <QRCodeSVG
            value={data}
            size={220}
            level={"H"}
          ...
          <motion.div 
            ...
            className="absolute left-0 right-0 h-0.5 bg-neon-green shadow-[0_0_10px_#39FF14] z-10"
          />
        </div>
      </div>

      <div className="text-center w-full">
        <h4 className="text-2xl font-black text-white tracking-tight">Acceso Encriptado</h4>
        {isWeb3 ? (
          <div className="mt-4 p-4 rounded-2xl bg-white/5 border border-white/5 text-left space-y-2">
             <div className="flex justify-between text-[10px] font-bold">
                <span className="text-slate-500 uppercase">Patient</span>
                <span className="text-white font-mono">{patientAddr.slice(0, 6)}...{patientAddr.slice(-4)}</span>
             </div>
             <div className="flex justify-between text-[10px] font-bold">
                <span className="text-slate-500 uppercase">Vault CID</span>
                <span className="text-cobalt font-mono truncate ml-4">{cid}</span>
             </div>
             <div className="flex justify-between text-[10px] font-bold">
                <span className="text-slate-500 uppercase">Sig Proof</span>
                <span className="text-neon-green font-mono">VERIFIED ✓</span>
             </div>
          </div>
        ) : (
      ...
      <div className="flex gap-4 w-full">
        <button className="flex-1 flex items-center justify-center gap-3 bg-white/5 hover:bg-white/10 text-white py-4 rounded-2xl transition-all font-bold border border-white/5">
          <Download className="w-5 h-5 text-cobalt" />
          PNG
        </button>
        <button className="flex-1 flex items-center justify-center gap-3 bg-white text-slate-900 py-4 rounded-2xl transition-all font-bold hover:scale-[1.05] shadow-xl">
